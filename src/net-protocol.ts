...
export interface RoomState {code:string;host:string;phase:'lobby'|'countdown'|'race'|'results';countdown:number;round:number;players:NetPlayer[];order:string[];props:{x:number;z:number;tilt:number}[];tick:number;}
export type ClientMessage=
 | {type:'create';name:string;kart:KartId}
