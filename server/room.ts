...
import {frameAt,nearestTrack,type Obstacle} from '../src/track';
...
export class Room {
 ...
 obstacles:Obstacle[]=[];
 constructor(public code:string,public host:string){this.resetProps();}
 resetProps(){this.obstacles=world.obstacles.map(o=>({x:o.x,z:o.z,radius:o.radius,movable:o.movable,object:new THREE.Object3D(),vx:0,vz:0}));}
 add(id:string,name:string,kart:KartId){
 ...
 respawn(p:Driver){if(this.phase!=='race'||p.race.finished||p.dnf||p.respawnCooldown>0)return;p.vehicle.reset(p.race.respawnProgress());p.respawnCooldown=1;}
 ...
 step(dt:number){
  ...
  for(const p of active){
   ...
   if(p.padCooldown<=0&&p.vehicle.speed>3&&near.distance<2.6&&world.boosts.some(t=>Math.abs(t-near.progress)<.011)){p.vehicle.padBoost=1.1;p.padCooldown=2;}
   world.coins.forEach((c,i)=>{if(!p.collected.has(i)&&Math.hypot(p.vehicle.x-c.x,p.vehicle.z-c.z)<1.8&&p.vehicle.y<2){p.collected.add(i);p.coins++;p.vehicle.boost=Math.min(1,p.vehicle.boost+.15);}});
  }
 ...
 snapshot():RoomState{
  ...
  return {code:this.code,host:this.host,phase:this.phase,countdown:this.countdown,round:this.round,players,order,tick:this.tick,props:this.obstacles.filter(o=>o.movable).map(o=>({x:o.x,z:o.z,tilt:o.object.rotation.z}))};
 }
}
